...
export function AccountAddresses() {
  ...
  return <AccountLayout>
    <div className="rounded-2xl border border-line p-6 dark:border-line-dark md:p-8">
      ...
      <form onSubmit={e => { e.preventDefault(); setSaved(true); setTimeout(() => setSaved(false), 2000) }} className="mt-6 max-w-lg space-y-4">
        ...
        <button type="submit" className="rounded-lg bg-burgundy px-6 py-3 text-sm font-semibold text-cream transition hover:bg-burgundy-dark">{saved ? t('تم الحفظ ✓', 'Saved ✓') : t('حفظ العنوان', 'Save address')}</button>
      </form>
    </div>
  </AccountLayout>
}