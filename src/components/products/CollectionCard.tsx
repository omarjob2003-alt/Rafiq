...
export function CollectionCard({ collection, index = 0 }: { collection: Collection; index?: number }) {
  ...
  return <motion.a href={collection.href} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .6, delay: index * .08 }} className="group relative block aspect-[3/4] w-[78%] shrink-0 snap-start overflow-hidden rounded-[16px] sm:w-full">
